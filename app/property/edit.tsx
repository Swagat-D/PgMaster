...
import roomsDataImport from '../../src/data/properties';
...
export default function EditRoom() {
  ...
  const roomsData: any[] = roomsDataImport;
  ...
  useEffect(() => {
    const id = roomId;
    if (id) {
      const room = roomsData.find(r => String(r.id) === id || String(r.roomNumber) === id);
      if (room) {
        setEditingId(room.id ?? id);
        setName(room.roomNumber ?? '');
        setBeds(room.bedCount != null ? String(room.bedCount) : '');
        setRent(room.pricePerBed != null ? String(room.pricePerBed) : '');
        setFloor(room.floor ?? '');
        setAmenities(Array.isArray(room.amenities) ? room.amenities.slice() : []);
        setImages(Array.isArray(room.images) ? room.images.slice() : []);
      }
    }
  }, [roomId]);

  const saveEdit = () => {
    if (!editingId) return router.back();
    try {
      const idx = roomsData.findIndex(r => String(r.id) === String(editingId));
      if (idx > -1) {
        roomsData[idx] = {
          ...roomsData[idx],
          roomNumber: name,
          bedCount: Number(beds) || roomsData[idx].bedCount,
          pricePerBed: rent ? Number(rent) : roomsData[idx].pricePerBed,
          floor: floor,
    ...
    router.replace(`/property/${editingId}`);
  };
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.rowTwo}>
          ...
          <View style={{ flex: 1, marginLeft: wp(2) }}>
            ...
            <TextInput value={beds} onChangeText={setBeds} placeholder="No of Beds" placeholderTextColor="#9CA3AF" keyboardType="numeric" style={styles.input} />
          </View>
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.photosRow, { paddingRight: wp(3) }]}>
          {images.map((uri: any, idx: number) => (
            <View key={idx} style={styles.imageWrapper}>
              <View style={styles.photoThumb}>
                <Image source={typeof uri === 'string' ? { uri } : uri} style={{ width: '100%', height: '100%' }} resizeMode="cover" />
              </View>
        ...
        <TouchableOpacity style={styles.addBtn} activeOpacity={0.9} onPress={saveEdit}>
          <Text style={styles.addBtnText}>Edit Room</Text>
        </TouchableOpacity>
